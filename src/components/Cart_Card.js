  ...
  const downCount = () => {
    if (count > 0.5) {
      ...
      !editChk
        ...
        : dispatch(editDownAmount(props.foodId));
    } else if (count > 0.1) {
      setCount(Number((count - 0.1).toFixed(1)));
      !editChk
        ? dispatch(setDownAmount(props.foodId))
  ...
  return (
    <React.Fragment>
      <CardContainer onClick={closeDelete}>
        <FoodCard style={{ transform: `translate( ${del ? "-28%" : "0"}, 0)` }}>
          <Grid>
            {/* 이름 */}
          ...
          {/* 카운트 */}
          <CountBox>
            <div onClick={downCount}>
              <IoIosArrowDropleft color="gray" size="18px" />
            </div>
            <Text
              lineheight="28px"
              m_lineheight="25px"
            ...
            <div onClick={upCount}>
              <IoIosArrowDropright color="gray" size="18px" />
            </div>
          </CountBox>
