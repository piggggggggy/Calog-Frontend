  ...
  reducers: {
    ...
    searchKeyword: (state, action) => {
      if (action.payload.data === undefined) {
        ...
        const filtered = action.payload.data.filter((food, idx) => {
          if (
            parseInt(food.kcal) >= action.payload.data.min &&
            parseInt(food.kcal) <= action.payload.data.max
          ) {
            return food;
          }
        });
    ...
    rangeFilter: (state, action) => {
      const filtered = state.list.filter((food, idx) => {
        if (
          food.kcal >= action.payload.min &&
          food.kcal <= action.payload.max
        ) {
          return food;
        }
      });
    ...
    addMostUsedKey: (state, action) => {
      let index = state.most.findIndex(
        (most) => most.keyword === action.payload
      );
      if (index === -1) {
