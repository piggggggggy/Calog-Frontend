  ...
  return function (dispatch, getState, { history }) {
    instance
      ...
      .catch((err) => {
        ...
        window.alert(
          "게시글 업로드에 오류가 발생했어요! 관리자에게 문의해주세요😿"
        );
        history.push("/");
      });
  };
...
export const addRecordDB = (type, url, memo, recordId, date) => {
  return function (dispatch, getState, { history }) {
    ...
    instance
      .post(`/api/record/${recordId}/urlContents`, {
        type: type,
        url: url,
        contents: dataMemo,
      })
  ...
  return function (dispatch, getState, { history }) {
    instance
      ...
      .catch((err) => {
        Sentry.captureException(`Catched Error : ${err}`);
        window.alert(
          "게시글 삭제에 오류가 발생했어요! 관리자에게 문의해주세요😿"
        );
      });
  };
};
  ...
  return function (dispatch, getState, { history }) {
    instance
      .put(`/api/record/${recordId}`, {
        date: date,
        foodList: typeFoodList,
        type: data_type,
        typeCalories: typeCalories,
      })
      .then((res) => {
        history.push(`/loading/calendar/${date}`);
      })
  ...
  return function (dispatch, getState, { history }) {
    ...
    instance
      ...
      .catch((err) => {
        Sentry.captureException(`Catched Error : ${err}`);
        window.alert(
          "기록을 불러오는데 오류가 발생했어요! 관리자에게 문의해주세요😿"
        );
        history.push("/");
      });
  };
  ...
  return function (dispatch, getState, { history }) {
    instance
      ...
      .catch((err) => {
        Sentry.captureException(`Catched Error : ${err}`);
        window.alert(
          "게시글 로드에 문제가 발생했어요! 관리자에게 문의해주세요😿"
        );
        history.push("/");
      });
  };
    ...
    instance
      ...
      .then((res) => {
        ...
        if (res.data?.length === 0) {
          ...
          dispatch(getRecord(record_list));
        }
        dispatch(isLoaded(true));
      })
      .catch((err) => {
        Sentry.captureException(`Catched Error : ${err}`);
        window.alert(
          "기록을 로드하는데 오류가 있어요! 관리자에게 문의해주세요😿"
        );
