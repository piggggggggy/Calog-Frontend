...
const food = createSlice({
  ...
  reducers: {
    ...
    editUpAmount: (state, action) => {
      const index = state.typeFood.findIndex(
        (food) => food.foodId === action.payload
      );
      if (state.typeFood[index].amount >= 0.5) {
        state.typeFood[index].amount += 0.5;
      } else {
        state.typeFood[index].amount = Number(
          (state.typeFood[index].amount + 0.1).toFixed(1)
        );
      }
    },
