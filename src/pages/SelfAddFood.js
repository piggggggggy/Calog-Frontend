...
const SelfAddFood = () => {
  ...
  const addFood = () => {
    ...
    const carbo =
      carboInput.current.value === "" ? undefined : carboInput.current.value;
    const sugars =
      sugarsInput.current.value === "" ? undefined : sugarsInput.current.value;
    const protein =
      proteinInput.current.value === ""
        ? undefined
        : proteinInput.current.value;
    const fat =
      fatInput.current.value === "" ? undefined : fatInput.current.value;
    const fattyAcid =
      fattyAcidInput.current.value === ""
        ? undefined
        : fattyAcidInput.current.value;
    const transFattyAcid =
      transFattyAcidInput.current.value === ""
        ? undefined
        : transFattyAcidInput.current.value;
    const unFattyAcid =
      unFattyAcidInput.current.value === ""
        ? undefined
        : unFattyAcidInput.current.value;
    const cholesterol =
      cholesterolInput.current.value === ""
        ? undefined
        : cholesterolInput.current.value;
    const natrium =
      natriumInput.current.value === ""
        ? undefined
        : natriumInput.current.value;
    const forOne =
      forOneInput.current.value === ""
        ? undefined
        : parseInt(forOneInput.current.value);
    const measurement =
    ...
    if (history.location.pathname.includes("onlyHQ")) {
      ...
      if (!name || !kcal) {
        ...
        if (result) {
          let cartForOne =
            forOneInput.current.value === "" ? "-" : forOneInput.current.value;
          let cartForMeasurement =
            measurementInput.current.value === ""
              ? "-"
              : measurementInput.current.value;
          dispatch(
    ...
    <React.Fragment>
      ...
      <Grid margin="8% 6%" m_margin="8% 6%">
        ...
        <Text size="17px" margin="0 0 2% 0" m_size="15px">
          음식명<span style={{ color: "#FD0000" }}>*</span>
        </Text>
        ...
        {/* 칼로리 */}
        <Text size="17px" margin="4% 0 2% 0" m_size="15px">
          칼로리<span style={{ color: "#FD0000" }}>*</span>
        </Text>
