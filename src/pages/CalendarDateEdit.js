  ...
  const editBtn = () => {
    if (typeFoodList?.length === 0) {
      const result = window.confirm(
        `식단이 삭제되면 ${data_type}에 대한 기록이 모두 사라져요! 삭제하시겠습니까?`
      );
      result
        ? dispatch(delRecordDB(recordId, date, data_type))
        : history.push(`/loading/calendar/${date}`);
    } else {
      dispatch(
        editRecordDB(recordId, date, typeFoodList, data_type, typeCalories)
      );
    }
  };
  ...
  return (
    <React.Fragment>
      ...
      <Grid margin="5% 0 0 8%" m_margin="5% 0 0 8%">
        <Text size="20px" bold m_size="15px">
          {data_type} 식단
        </Text>
      </Grid>
