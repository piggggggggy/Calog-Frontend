...
const CalendarDate = () => {
    ...
    if (fileList?.length > 0) {
      for (let i = 0; i < fileList?.length; i++) {
        ...
        try {
          ...
          window.alert(
            "앗, 게시글 업로드에 오류가 있어요! 관리자에게 문의해주세요😿"
          );
        }
      }
    } else {
  ...
  const [btn, setBtn] = useState({
    color:
      previewImg?.length === 0 || previewImg.files?.length === 0
        ? "#9E9E9E"
        : theme.color.light,
    fontColor:
      previewImg?.length === 0 || previewImg.files?.length === 0
  ...
  const delRecord = async () => {
    ...
    result
      ? dispatch(delRecordDB(record_list?.recordId, SelectDate, type))
      : history.push(`/loading/calendar/${SelectDate}`);
  };
  ...
  const delFoodBtn = () => {
    const result = window.confirm(
      `식단이 삭제되면 ${type}에 대한 기록이 모두 사라져요! 삭제하시겠습니까?`
    );
    result
      ? dispatch(delRecordDB(record_list?.recordId, SelectDate, type))
      : history.push(`/loading/calendar/${SelectDate}`);
    ...
    <React.Fragment>
      <Wrap>
        ...
        <TopBack>
          ...
          {/* bmr info */}
          <BmrInfo>
            <Text color={"rgba(158, 135, 55, 0.6)"}>
              {date} 기초대사량 {record_bmr}kcal
            </Text>
          </BmrInfo>
        ...
        {same_food?.length === 0 ? (
          ...
          <Grid width="80.9%" margin="4% auto 0 auto" m_margin="4% auto 0 auto">
            {same_food?.map((r, idx) => {
              return (
                <CalendarDetail_Food data_type={type} key={r._id} {...r} />
              );
            })}
          </Grid>
        )}
          ...
          <React.Fragment>
            ...
            {memo_list?.length !== 0 ? (
              <Grid
                margin="4% 9.7% 8% 9.7%"
                width="81%"
                m_margin="4% 9.7% 8% 9.7%"
                cursor="default"
              >
                {memo_list?.map((r, idx) => {
                  return (
                    <Text margin="0 0 3% 0" size="15px" m_size="13px">
                      {r}
                    </Text>
                  );
                })}
        ...
        same_food?.length !== 0 ? (
          ...
          >
            <Text size="16px" bold color={fontColor} cursor="pointer">
              기록하기
            </Text>
          </Button>
        )}
